...
- **Status History:**
  ...
  - 2026-07-08 — Done (live-validated on PR #32: pushed a deliberately broken `harmonic-custom/config.yml` (bad indentation) → `lint-and-test` failed with a clear syntax error; reverted the commit → `lint-and-test`, `secret-scan`, `docs-links`, and CodeQL `analyze` all passed).
